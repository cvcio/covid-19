<template>
	<v-app-bar app class="elevation-0 top-app-bar" :color="$vuetify.breakpoint.smAndDown ? 'white' : 'rgba(237, 32, 56,0.85)'">
			...
			<!-- <v-row align="center" justify="start" class=""> -->
				...
				<template v-else>
					<v-col align-self="center" class="ma-0 py-0 white shrink d-flex">
						<v-app-bar-nav-icon @click="$store.commit('setSidebar', !sidebar)" v-if="$vuetify.breakpoint.smAndDown" class="mt-2">
							<v-icon small color="primary">fa-sliders-h</v-icon>
						</v-app-bar-nav-icon>
					...
					<v-col align-self="center" class="ma-0 py-2 white--text" sm="auto">
						<v-row align="center">
