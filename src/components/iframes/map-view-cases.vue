<!--
	"el": "N2_1 (New 3a) - Χάρτης Κρουσμάτων άνα Π.Ε.",
	"en": "N2_1 (New 3a) - Cases Map per Regional Unit"
-->
	...
	<v-container fluid class="max-width pa-0">
		<v-container fluid class="pa-0">
			<v-row no-gutters class="no-events">
				<div id="mapc-container" class="d-block blue-grey lighten-4"
					:class="$vuetify.breakpoint.smAndDown ? 'mobile' : 'desktop'">
					<template>
						<div id="map_cases" class="d-block events" :class="
							[
						...
						">
										...
										<template>
											<v-card-title class="pa-2 subtitle-2">
												<span class="text-uppercase">
													{{ $t(point._cases.slug) | normalizeNFD }}:
													<!-- {{ point["name_" + locale.code] | normalizeNFD }} -->
												</span>
											</v-card-title>
											<v-divider />

											<template v-for="item in point._cases.cases_tooltip">
							...
							<div class="legend" :class="$vuetify.breakpoint.smAndDown ? 'center' : ''">
								...
								<v-row no-gutters>
									<v-col class="caption font-weight-bold">{{
										new
																			Intl.NumberFormat('el-GR').format(min.toFixed(2))
									}}
									</v-col>
...
export default {
	...
	mounted() {
		...
		if (this.geoc && this.geoc.features) {
			// console.debug('geo loaded from storage');
			this.load();
		} else {
			// console.debug('request geojson');
			this.$store.dispatch('internal/getGeoRegionalUnitsC')
				.then(() => {
					this.load();
				});
		}
	...
	methods: {
		...
		beforeDestroy() {
			...
			this.map.off('mouseenter', 'cases', this.onMouseEnter);
			this.map.off('mouseleave', 'cases', this.onMouseLeave);
			this.map = null;
		},
		reloadMap() {
			const values = this.geoc.features.map(m => {
				if (m.properties._cases) {
					const cases = m.properties._cases.cases.find(obj => { return obj.year == this.mapYear; });
					return m.properties._cases.pop_11 > 0 ? (cases.number / m.properties._cases.pop_11) * 100000 : 0;
				} else {
					return 0;
				}
			});
			...
			this.geoc.features.map(m => {
				...
				if (m.properties._cases) {
					...
					const cases = m.properties._cases.cases.find(m => { return m.year == this.mapYear; });
					m.properties.color = m.properties._cases.pop_11 > 0 ? palette((cases.number / m.properties._cases.pop_11) * 100000) : palette(0);
					m.properties.opacity = cases.number > 0 ? 0.9 : 0.6;
			...
			if (this.map) {
				this.map.getSource('cases').setData(this.geoc);
				this.updateLayers();
			} else {
				this.draw();
			}
		...
		loadCovidCases() {
			this.$store.dispatch('external/getRegionalUnitsDataByYear', { from: "", to: "" })
				.then(res => {

					const values = res.map(m => {
						const cases = m.cases.find(obj => { return obj.year == this.mapYear; });
						return m.pop_11 > 0 ? (cases.number / m.pop_11) * 100000 : 0;
					});
					...
					this.geoc.features.map(m => {
						...
						const obj = res.find(o => o.regional_unit_id == m.properties.imedd_id);
						if (obj) {

							m.properties.active = true;
							m.properties._cases = { ...obj };
							m.properties._cases.slug = `${obj.slug}_ru_e`;

							const cases = m.properties._cases.cases.find(m => { return m.year == this.mapYear; });
							m.properties.color = m.properties._cases.pop_11 > 0 ? palette((cases.number / m.properties._cases.pop_11) * 100000) : palette(0);
							m.properties.opacity = cases.number > 0 ? 0.9 : 0.6;

							var cases_tooltip = obj.cases.map((item) => {
								return {
									...item,
									per_hundred: (item.number / m.properties._cases.pop_11) * 100000
								}
							})
							m.properties._cases['cases_tooltip'] = cases_tooltip.sort((a, b) => { return a.year - b.year; });
						} else {
							m.properties.active = false;
						}
		...
		updateLayers(mapSource) {
			if (!this.map) return;
			this.map.setLayoutProperty('cases', 'visibility', 'visible');
		},
		load() {
			this.loadCovidCases();
		},
		...
		addLayerCovid() {
			...
			this.map.on('mousemove', 'cases', this.onMouseEnterC);
			this.map.on('mouseleave', 'cases', this.onMouseLeaveC);
		},
		onMouseEnterC(e) {
			if (!this.map) return;
			...
			if (features.length > 0) {
				var pointFeatures = features.find(obj => {
					return obj.source === 'cases';
				});
				this.map.setFilter('cases-border',
					['in', 'NAME', pointFeatures.properties.NAME]
				);
				this.point = pointFeatures.properties;
				this.point._cases = JSON.parse(this.point._cases);
